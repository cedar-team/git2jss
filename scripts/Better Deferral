...
NOTE: There is a critical update for iTerm2. This will quit out of iTerm2 if its already running.
Please use the defer timer to chose the best time to update. If you have any questions please reach out to the Platform team.
"""
...
def main():
    ...
    if args.mode == 'prompt':
        ...
        if os.path.exists(ld_path):
            with open(ld_path, 'rb') as _fh:
                current_settings = plistlib.readPlist(_fh)
            _today = datetime.datetime.today()
            pop_time = datetime.datetime(
                _today.year, _today.month,
                current_settings['StartCalendarInterval']['Day'],
                current_settings['StartCalendarInterval']['Hour'],
                current_settings['StartCalendarInterval']['Minute'])
            if datetime.datetime.now() > pop_time:
                print "We have passed the expiration time. Firing!"
                if subprocess.call(current_settings['ProgramArguments']) != 0:
                    print "Failed to run jamf. Will try again"
                    sys.exit(1)
                print "We did the thing! Hurray!"
                sys.exit(0)
